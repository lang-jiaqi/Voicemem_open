...
loop should be able to add long-term memory by changing ONE line --
    ...
    from voicemem_core import inject
...
def build_memory_context(result: Any, max_rb: int = 3) -> str:
    ...
    prestimulus = getattr(result, "prestimulus_text", None)
    if prestimulus:
        lines.append("")
        lines.append(prestimulus)
...
# (`from voicemem_core import inject` + `inject(messages)` -- no visible setup;
