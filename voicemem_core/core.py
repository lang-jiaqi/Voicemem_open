...
from voicemem_core.leftbrain.cognitive_graph.slot_v2 import SLOT_RELATIONS
from voicemem_core.leftbrain.cognitive_graph.query_slot_classifier import QueryClassification
from voicemem_core.leftbrain.local_memory_store import MemorySearchHit
...
def _rb_emotion_trait_hit(rb_graph, user_id: str, emotion: str | None) -> "RightBrainHit | None":
    ...
    if not emotion:
        return None
    from voicemem_core.rightbrain.anchor_router import normalize_emotion_strict
...
def _rb_relation_hits(rb_graph, user_id: str, anchors) -> list["RightBrainHit"]:
    ...
    from voicemem_core.rightbrain.anchor_router import _ENTITY_TYPE_TO_ANCHOR
...
class AudioPerception:
    ...
    是 voicemem 核心与语音层结合的数据接缝。speaker/emotion 可能被感知
    结果覆盖（如声纹识别出的 person_id、韵律情绪），其余字段供 Ingest
    组装 ctx 与写 audiomem 标签使用。"""
    speaker: str
    emotion: str
...
class VoiceMem:
    ...
    def _get_repo(self):
        with self._lock:
            if "repo" not in self._cache:
                from voicemem_core.leftbrain.cognitive_graph import CognitiveAnnotator, CognitiveAnnotatorConfig
                from voicemem_core.leftbrain.local_memory_store import OpenAILocalEmbedder, OpenAILocalEmbedderConfig
                from voicemem_core.leftbrain.memory_repository_v2 import LeftBrainMemoryRepositoryConfig, LeftBrainMemoryRepositoryV2
    ...
    def _get_rb_repo(self):
        with self._lock:
            if "rb_repo" not in self._cache:
                from voicemem_core.leftbrain.cognitive_graph import CognitiveGraphStore
                from voicemem_core.rightbrain import ExperienceRepository
    ...
    def _get_registry(self):
        with self._lock:
            if "registry" not in self._cache:
                from voicemem_core.voice_input import VoiceprintRegistry
    ...
    def _get_prestimulus(self):
        with self._lock:
            if "prestimulus" not in self._cache:
                from voicemem_core.prestimulus import PreStimulusLayer
    ...
    def _get_clap_env_detector(self):
        with self._lock:
            if "clap_env_detector" not in self._cache:
                from voicemem_core.environment_detector_clap import CLAPEnvironmentDetector
    ...
    def _get_trigger_store(self):
        with self._lock:
            if "trigger_store" not in self._cache:
                from voicemem_core.scene_trigger import SceneTriggerStore
    ...
    def _get_audio_archive(self):
        with self._lock:
            if "audio_archive" not in self._cache:
                from voicemem_core.audio_archive import AudioArchive
    ...
    def _get_vp_store(self):
        with self._lock:
            if "vp_store" not in self._cache:
                ...
                from voicemem_core.voice_config import VoiceStoreConfig
    ...
    def _get_music_store(self):
        with self._lock:
            if "music_store" not in self._cache:
                from voicemem_core.music_memory import MusicMemoryStore
    ...
    def _get_routine_store(self):
        with self._lock:
            if "routine_store" not in self._cache:
                from voicemem_core.routine_memory import RoutineStore
    ...
    def _get_place_store(self):
        with self._lock:
            if "place_store" not in self._cache:
                from voicemem_core.place_memory import PlaceMemoryStore
    ...
    def CreateSceneTrigger(self, text: str) -> dict:
        ...
        """
        from voicemem_core.scene_trigger import parse_trigger_intent
    ...
    def _get_dynamic_slot_store(self):
        with self._lock:
            if "dynamic_slot_store" not in self._cache:
                from voicemem_core.leftbrain.slot_split import DynamicSlotStore
    ...
    def _get_graph_entity_store(self):
        with self._lock:
            if "graph_entity_store" not in self._cache:
                from voicemem_core.leftbrain.slot_split import GraphEntityStore
    ...
    def _get_rb_graph_store(self):
        with self._lock:
            if "rb_graph_store" not in self._cache:
                from voicemem_core.rightbrain import RightBrainGraphStore
    ...
    def _get_session_tracker(self):
        with self._lock:
            if "session_tracker" not in self._cache:
                from voicemem_core.session_tracker import SessionTracker
    ...
    def _get_subgraph_manager(self):
        ...
        with self._lock:
            if "subgraph_manager" not in self._cache:
                from voicemem_core.leftbrain.slot_split import SubgraphManager
    ...
    def _get_attribution_manager(self):
        ...
        with self._lock:
            if "attribution_manager" not in self._cache:
                from voicemem_core.rightbrain import AttributionManager
    ...
    def _llm_json(self, prompt: str) -> str:
        try:
            ...
            from voicemem_core.cost_log import log_usage
    ...
    def _widen_for_time_question(self, query: str, final_ids: set[str]) -> set[str]:
        ...
        if not final_ids:
            return final_ids
        from voicemem_core.leftbrain.local_memory_store import time_question_kind
    ...
    def _normalize_slot_name(
        ...
        from voicemem_core.leftbrain.slot_split.split_manager import cosine_sim
    ...
    def _llm_tag_memories(self, text: str, memory_ids: list[str]) -> list[str]:
        ...
        """
        import json as _json
        from voicemem_core.leftbrain.cognitive_graph.slot_v2 import ALL_SLOT_V2_VALUES, SLOT_V2_DESCRIPTIONS
    ...
    def Classify(self, query: str) -> QueryClassification:
        ...
        """
        from voicemem_core.leftbrain.cognitive_graph.query_slot_classifier import (
    ...
    def ArchiveColdMemories(
        ...
        from voicemem_core.leftbrain.cognitive_graph.store import ARCHIVE_HEAT_THRESHOLD
    ...
    def Search(
        ...
        if scene_filter is None:
            from voicemem_core.scene_classifier import infer_scene_from_text
        ...
        def _run_rb() -> list[RightBrainHit]:
            if disable_right_brain:
                return []
            try:
                from voicemem_core.rightbrain.types import CurrentSignals
                rb_repo = self._get_rb_repo()
        ...
        current_scene = ""
        try:
            from voicemem_core.scene_classifier import SceneTag, scene_to_response_directive
    ...
    def preprocess(
        ...
        """流式预处理：Ingest() 推理路径的第 ① 步，一个独立、可单独调用的公开接缝。

        把一轮（流式切分出来的）音频跑过所有声学感知模块——声学场景 / 背景
        音乐 / 异常环境音 / 说话人声纹 / 自我介绍绑定 / 韵律情绪——外加纯文本
        模式下的自我介绍绑定与情绪兜底，汇总成一个 AudioPerception。这是
        『voicemem 核心如何与语音结合』的唯一入口：语音那边只要给出
        text（+ 可选 audio_path），剩下的感知全在这里发生；返回后
        Ingest() 只负责把结果组装进 ctx、交给 _finish_ingest 写记忆。

        独立公开是为了让语音管线能把"预处理"和"写记忆"解耦：调用方可以先
        ``p = vm.preprocess(text, audio_path=...)`` 拿到场景/说话人/情绪等信号
        用于实时应答，再决定要不要 Ingest。纯结构计算，不写任何记忆；会就地
        更新声纹注册表 / voiceprint store 等共享状态。
        """
        environment = ""
        environment_hint = ""
        ...
        if audio_path is not None:
            ...
            if self._enable_scene or self._enable_music or self._enable_abnormal_sound:
                try:
                    from voicemem_core.scene_classifier import classify_scene
    ...
    def Ingest(
        ...
        """
        import time, uuid
        from voicemem_core.voice_input import VoiceInput, VoiceContent, ingest_voice_input
    ...
    def _finish_ingest(self, ctx: dict) -> dict:
        ...
        if scene_tag and result.memory_ids:
            ...
            try:
                from voicemem_core.scene_trigger import check_and_fire
            ...
            if _fire_result is not None and _fire_result.scene_changed:
                try:
                    ...
                    from voicemem_core.routine_memory import bucket_label
        ...
        if emotion:
            try:
                from voicemem_core.rightbrain.types import MemoryAnchor
                rb_repo = self._get_rb_repo()
