"""voicemem 顶层门面 = 编排器（mem0 式）：一个类 VoiceMem = 左脑 + 右脑 + 音频感知 + 一组可换的能力(utils)。
...
本类直接持有并构造三个自包含组件（self._left/_right/_audio），编排它们完成
Search/Ingest 等完整 pipeline；面向用户的小写便捷方法（ingest/search/…）委托给
大写编排方法。既是 mem0 式编排器，也是按 mode 懒加载 util 的门面。
...
class VoiceMem:
    ...
    def ingest(self, text, audio=None, **kw): return self.Ingest(text, audio_path=audio, **kw)
    def search(self, query, **kw):            return self.Search(query, **kw)
    def classify(self, query):                return self.Classify(query)
