...
from voicemem_core._graph_common import cosine as _cosine, new_id as _new_id, utc_iso as _utc_iso
...
class GraphEntityStore:
    ...
    def _ensure_schema(self) -> None:
        with self._conn() as c:
            ...
            # 只有跨版本升级的旧库会踩这个顺序错误。
            try:
                c.execute("ALTER TABLE query_activations ADD COLUMN session_id TEXT")
            except sqlite3.OperationalError:
                pass  # column already exists — skip
            c.execute("CREATE INDEX IF NOT EXISTS idx_qa_user_session ON query_activations(user_id, session_id)")
    ...
    def record_query_activation(
        ...
        with self._conn() as c:
            c.executemany(
                "INSERT INTO query_activations (id, user_id, query_id, entity_id, session_id, created_at)"
                " VALUES (?,?,?,?,?,?)",
    ...
    def compute_rho(
        ...
        with self._conn() as c:
            if session_id is not None:
                rows = c.execute(
                    "SELECT query_id, entity_id FROM query_activations WHERE user_id=? AND session_id=?",
                    (user_id, session_id),
                ).fetchall()
            else:
                rows = c.execute(
                    "SELECT query_id, entity_id FROM query_activations WHERE user_id=?",
                    (user_id,),
                ).fetchall()
        if not rows:
            return 0.0
