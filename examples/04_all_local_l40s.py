...
async def warmup(stream):
    """模型全是懒加载的：不预热的话第一轮要现加载 E5 / FunASR / silero / VoxCPM，
    用户对着 [ready] 说完第一句要等好几秒。"""
    await asyncio.to_thread(vm.search, "预热")           # E5 + 向量库
    await stream.feed(b"\x00" * 320)                     # FunASR + silero
